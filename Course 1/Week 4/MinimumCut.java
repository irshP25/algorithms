...
public class MinimumCut {
    ...
    private static Graph Graphcreation(int[][] array) {
        Graph gr = new Graph();
    ...
    public static void main( String[] args ) {
        int[][] arr = getArray( "C:/Users/hp/Desktop/kargermincut.txt" );
        ...
        for( int i = 0; i < iter; i++ ) {
            Graph gr = Graphcreation( arr );
            //printGraph( gr );
            int mincount = minCut( gr );
            min = Math.min( min, mincount );

            Integer counter;
            if( ( counter = statistics.get( mincount ) ) == null ) {
                counter = 0;
            }
            statistics.put( mincount, counter+1 );
        }
        System.out.println( "Mincuts: " + min );
    }
